...
class AICrawler {
    constructor() {
        this.browser = null;
        this.maxRetries = 3;
        this.timeout = parseInt(process.env.BROWSER_TIMEOUT || '120000');
    ...
    async randomDelay() {
        ...
        // 额外随机延时
        const randomDelay = Math.floor(Math.random() * (ANTI_CRAWL_CONFIG.delayMax - ANTI_CRAWL_CONFIG.delayMin) +
    ...
    async simulateMouseMove(page) {
        if (!ANTI_CRAWL_CONFIG.enableMouseMove)
            return;
    ...
    // 模拟随机滚动
    async simulateRandomScroll(page) {
        if (!ANTI_CRAWL_CONFIG.enableRandomScroll)
            return;
        // 随机滚动几次
        const scrolls = Math.floor(Math.random() * 3) + 1;
        for (let i = 0; i < scrolls; i++) {
            const scrollY = Math.floor(Math.random() * 300) + 100;
            await page.evaluate((y) => { globalThis.scrollBy(0, y); }, scrollY);
    ...
    async crawlWithBrowser(config, question, brandName) {
        if (!this.browser)
    ...
    async waitForAnswer(page, selector) {
    ...
    parseMentions(answer, brandName) {

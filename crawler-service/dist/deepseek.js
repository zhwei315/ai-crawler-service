...
class DeepSeekAnalyzer {
    ...
    buildAnalysisPrompt(brandName, questions) {
    ...
    parseAnalysisResult(platform, brandName, questions, analysis) {
        // 尝试从分析文本中提取JSON
        let scores = {
            overallScore: 70,
            visibilityScore: 75,
        ...
        // 构建问题结果
        const positiveQuestions = questions.slice(0, 10).map(q => {
        ...
        const negativeQuestions = questions.slice(10).map(q => {
            const mentioned = q.mentions.some(m => m.mentioned);
            const brands = q.mentions.filter(m => m.mentioned).map(m => m.brand).join(',');
            return {
                question: q.question,
    ...
    ruleBasedAnalysis(platform, brandName, questions) {
        // 基于规则的分析
        const positiveQuestions = questions.slice(0, 10);
        const negativeQuestions = questions.slice(10);
        const positiveMentioned = positiveQuestions.filter(q => q.mentions.some(m => m.mentioned)).length;
        const negativeMentioned = negativeQuestions.filter(q => q.mentions.some(m => m.mentioned)).length;
    ...
    extractSourcesFromAnswer(answer) {
        const sources = [];
        const patterns = [
            { name: '知乎', pattern: /知乎/ },
